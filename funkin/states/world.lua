...
function WorldState:enter()
	...
	local gltf = Menori.glTFLoader.load('assets/models/player.glb')
	if not gltf then
...
function WorldState:update(dt)
	if controls:down("world_left") then
		WorldState.player.position.x = WorldState.player.position.x + --[[dt * ]]8
	end
	if controls:down("world_right") then
		WorldState.player.position.x = WorldState.player.position.x - --[[dt * ]]8
	end
	if controls:down("world_up") then
		WorldState.player.position.y = WorldState.player.position.x + --[[dt * ]]8
	end
	...
	self.worldCanvas:renderTo(function()
		love.graphics.setBackgroundColor(1, 0.5, 0)
		Menori.app:set_scene('world')
		Menori.app:render()
		end)

	self.worldSprite:loadTexture(love.graphics.newImage(self.worldCanvas:newImageData()))
	self.worldSprite:updateHitbox()
end
